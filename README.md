...
used to verify IAM enforcement.

---

## Quick start

Prerequisites: Docker, [uv](https://docs.astral.sh/uv/), Terraform, trivy, tflint, and a
[LocalStack](https://localstack.cloud) auth token (free Hobby tier).

The auth token is the one blocking prerequisite — since version 2026.03.0 the container will
...
**One thing to note:** LocalStack Hobby has no state persistence, so `make down` destroys the buckets
while Terraform state still records them. `make tf-apply` must follow every `make up` — it drops the missing resources from state, and recreates them.
...
Quarantine is written to the landing bucket rather than curated. The finance_analyst role can only read curated, so rejected records, which hold the same sensitive data as valid ones, stay out of its reach. Files live under quarantine/finance/transactions/dt=2026-08-19/rejects.csv for inspection and replay.

### Infrastructure

For each specified domain, a landing and a curated bucket are created, each with SSE-S3 encryption, all public access blocked, versioning enabled, and lifecycle rules. Landing files expire after 30 days;
curated objects are not expired but any superseded versions expire after 30 days.
...
- Curated writes delete the target date partition before uploading, across all categories
  within it — so a category that disappears from the source does not leave a stale file behind.
- Only the run date's partition is touched.
- Data is generated using a fixed RNG seed, so the same date always produces identical input.
  Without that, a test couldn't distinguish a real duplicate from random variation.

### Failure handling

The pipeline treats different failures differently, based on whether retrying could plausibly change the outcome.

- **Retries are only applied to network tasks** (fetch, S3 reads and writes). These can fail transiently and often succeed on the next attempt. The quality gate is deterministic — if it fails once it will fail again with the same input, so it doesn't retry.
- **Bad rows are quarantined, not fatal.** A row that fails validation is written to `quarantine/…/rejects.csv` with a reason attached, and the run continues. One malformed row does not stop the pipeline.
- **An empty quarantine file is still written** when there are no rejects, so a missing file unambiguously means the run did not complete rather than being ambiguous between "clean run" and "never ran".
- **Curated waits on quarantine** via a Prefect `wait_for` dependency. If the rejects file fails to persist after retries, the curated write is skipped entirely — a "successful" run has both files or neither, never one without the other.
...
**The policies were verified against real AWS.** LocalStack's free tier does not enforce IAM. `make test-aws` assumes the analyst role and asserts it can list curated, cannot list landing, and cannot write to curated:
...
One line in `part1_infrastructure/local/terraform.tfvars`:
...
Two roles is about forty lines of HCL, so they live in each environment's `main.tf` rather than
a module. If this grew to a lot more, we might consider a `roles` variable holding a map of
role name to `{ domains, layers, access }` and a single `for_each`.

### Adding another dataset

`s3.py`, `quality.py` and `config.py` are essentially domain-agnostic and can be reused easily.
The coupling to transactions is concentrated in `transform.py`, `ingest.py` and the flow.
A future step would be generalising these.

### Adding another curated model

The aggregation lives in `sql/daily_category_spend.sql`. A second model needs a second SQL file
and the partition columns and prefix parameterised — currently both are inline in
`transform.py`. At three or four models, it would probably make sense to switch to dbt.
...
| `make tf-plan-aws` / `tf-apply-aws` / `tf-destroy-aws` | The real AWS environment |
...
Whitespace and EOF fixes, YAML validation, private key detection, large file detection, `ruff`
lint and format, `terraform fmt`, and `tflint`.

`terraform validate` is deliberately not a hook — it requires an initialised directory and
so fails on a fresh clone. It runs via `make tf-validate` instead, and covers both environments.

`trivy config` fails on HIGH and above. Two findings are accepted rather than fixed, with
reasons in `.trivyignore`. The gate was verified by temporarily removing an exception and
confirming the scan fails.
...
| `test_quality.py` | Each defect class is classified correctly; rejected rows keep every original field; a row with two problems reports both |
| `test_idempotency.py` | Two runs of a date produce identical curated output; the landing object is replaced not duplicated; other dates are untouched |
| `test_iam.py` | The analyst role can read curated, cannot read landing, cannot write to curated |

Two types of defect are caught by the gate: structural (missing primary key, non-numeric
